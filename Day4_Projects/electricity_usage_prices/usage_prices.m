...
prices(isnan(prices)) = mean(mean(prices));
...
% Extract prices data for residential, commercial, and industrial
residential_price = prices(:, 1);
commercial_price = prices(:, 2);
industrial_price = prices(:, 3);

% Data is of 23 years from 1991 to 2013
years = (1991:2013)';

% Plot usage and prices of all three types on two different
% graphs within one frame using grid
...
% Second plot
subplot(2, 1, 2);
% Plot residential price with blue dashed line
plot(years, residential_price, 'b--');

% Hold the graph to plot the data on same graph
% otherwise it creates new graph
hold on;

% Plot commercial price with black dotted line
plot(years, commercial_price, 'k:');

% Plot industrial price with magenta dash-dot line
plot(years, industrial_price, 'm-.');

% Give title to graph
title("Electricity Prices(US cents per kWh)");
